...
class FeedHealthMonitor:
    """Comprehensive feed health monitoring service."""
    ...
    def check_feed_health(self, feed: Dict) -> Dict:
        ...
        try:
            # Prepare headers
            headers = {
                'User-Agent': 'SentinelForge-HealthMonitor/1.0',
                'Accept': '*/*'
            }
            ...
            if method == 'HEAD':
                ...
                response = self.session.get(url, headers=headers, timeout=30, allow_redirects=True, stream=True)
        ...
        self.logger.debug(f"Health check result for {feed_name}: {status} ({response_time_ms}ms)")
    ...
    def run_health_check(self, feed_id: Optional[int] = None, checked_by: int = 0) -> Dict:
        """Run health check for all feeds or specific feed."""
        ...
        for feed in feeds:
            try:
                ...
                if not self.log_health_result(health_result, checked_by):
                    self.logger.warning(f"Failed to log health result for feed {feed['name']}")
                
            except Exception as e:
                self.logger.error(f"Health check failed for feed {feed['name']}: {e}")
        ...
        self.logger.info(f"Health check completed: {healthy_feeds}/{total_feeds} feeds healthy")
        return result
    ...
    def start_cron_scheduler(self, interval_minutes: int = 1):
        ...
        try:
            ...
            self.logger.info(f"Cron scheduler started with {interval_minutes}-minute interval")
            return True
...
def run_startup_health_check():
    ...
    if result.get("success"):
        ...
        print(f"   📊 {summary['healthy_feeds']}/{summary['total_feeds']} feeds healthy ({summary['health_percentage']}%)")
        ...
        if unhealthy:
            ...
            for feed in unhealthy:
                print(f"      - {feed['feed_name']}: {feed['status']} ({feed.get('error_message', 'No details')})")
    else:
...
def main():
    ...
    parser.add_argument("--check-now", action="store_true", help="Run one-time health check")
    parser.add_argument("--start-cron", action="store_true", help="Start cron scheduler")
    parser.add_argument("--interval", type=int, default=1, help="Cron interval in minutes (default: 1)")
    parser.add_argument("--feed-id", type=int, help="Check specific feed ID")
    parser.add_argument("--startup", action="store_true", help="Run startup health check")
    ...
    if args.check_now:
        ...
        if result.get("success"):
            ...
            for feed in result["feeds"]:
                ...
                print(f"   {status_emoji} {feed['feed_name']}: {feed['status']} ({feed['response_time_ms']}ms)")
