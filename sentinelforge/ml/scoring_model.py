...
EXPECTED_FEATURES = (
    ...
    + ["feed_count"]
    + ["has_country", "country_high_risk", "country_medium_risk", "has_geo_coords"]  # IP features
    + ["has_registrar", "has_creation_date"]  # Domain features
    + ["url_length", "dot_count", "has_ip_in_url"]  # URL features
    + ["contains_&", "contains_?", "contains_=", "contains_.", "contains_-", "contains__", "contains_~", "contains_%", "contains_+"]  # URL special chars
    + ["hash_length"]  # Hash features
    + ["has_summary", "summary_length", "from_threat_feed", "from_url_feed", "from_test_feed"]  # General features
)
...
# --- Feature Extraction ---
def extract_features(ioc_type: str, source_feeds: List[str], ioc_value: str = "", enrichment_data: Dict = None, summary: str = "") -> Dict[str, Any]:
    """
    ...
    if normalized_type == 'ip':
        ...
        if 'country' in enrichment_data and enrichment_data['country']:
            ...
            features['country_high_risk'] = 1 if country in ['russia', 'china', 'iran', 'north korea'] else 0
            features['country_medium_risk'] = 1 if country in ['ukraine', 'belarus', 'romania'] else 0
            
        # Latitude/longitude features
        if 'latitude' in enrichment_data and enrichment_data['latitude'] and 'longitude' in enrichment_data and enrichment_data['longitude']:
    ...
    if normalized_type == 'url':
        ...
        # Check for IP in URL
        features['has_ip_in_url'] = 1 if any(c.isdigit() for c in ioc_value.split('.')) else 0
...
def predict_score(features: Dict[str, Any]) -> float:
    ...
    try:
        ...
        with open(os.devnull, 'w') as f, contextlib.redirect_stderr(f):  # Suppress warnings
